...
def _chart(results: list[RunResult], out_path: Path) -> None:
    ...
    for ax, (workload, metric, ylabel) in zip(axes.flat, panels):
        ...
        for strategy in STRATEGIES:
            ...
            if xs:
                # Direct label at the end of the line: identity is never carried
                # by colour alone. Text takes an ink token, not the series hue.
                ax.annotate(
                    strategy,
                    xy=(xs[-1], ys[-1]),
                    xytext=(6, 0),
    ...
    hot_p = next(r for r in results if r.workload == "hot" and r.strategy == "pessimistic" and r.concurrency == max(levels))
    hot_o = next(r for r in results if r.workload == "hot" and r.strategy == "optimistic" and r.concurrency == max(levels))
    fig.text(
        0.055,
        0.938,
        f"PostgreSQL 16, {hot_p.transactions} transactions per point, service layer "
        f"called directly (no HTTP).  At {max(levels)} writers on the hot account: "
        f"{hot_p.throughput_tps:.0f} tps pessimistic vs {hot_o.throughput_tps:.0f} tps "
        f"optimistic, with {hot_o.retries_total} retried transactions.",
        color=INK_SECONDARY,
        fontsize=8.5,
        ha="left",
    )
    ...
    fig.tight_layout(rect=(0.02, 0.045, 0.98, 0.915))
